...
{name = "hybrid+"
  initial <- expand.grid(
    alpha.pos.chosen = seq(0.05, 1, by = 0.05),
    beta =  seq(0.25, 15, by = 0.25),
    alpha.neg.chosen = seq(0.05, 1, by = 0.05),
    alpha.pos.nonchosen = seq(0.05, 1, by = 0.05),
    alpha.neg.nonchosen = seq(0.05, 1, by = 0.05)) %>% 
    as.list()
  rmodel[[name]] <- getModelMice(initial) %>%
    mutate(name = name,
           aic = calculateAIC(length(initial), as.numeric(value)))}
...
# 4. expand.grid(alpha.pos = seq(0.05, 1, by = 0.05), 
#                beta = seq(0.25, 15, by = 0.25), 
#                storage.pos = seq(0.05, 1, by = 0.05), 
#                storage.neg = seq(0.05,1, by = 0.05), 
...
# 1. expand.grid(alpha.pos = seq(0.05, 1, by = 0.05), 
#                beta = seq(0.25, 15, by = 0.25), 
#                storage.pos = seq(0.05, 1, by = 0.05), 
#                storage.neg = seq(0.05, 1, by = 0.05), 
#                storage.neutral = seq(0.05, 1, by = 0.05),
#             alpha.neg = seq(0.05, 1, by = 0.05)) %>% as.list()
model <- function(par, a) {
  a = a[with(a, order(start)), ]
  nll = 0
  if (par[1] < 0 | par[1] > 1 |
